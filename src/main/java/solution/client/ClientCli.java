...
public class ClientCli implements IClientCli {
    ...
    public LoginResponse login(String username, String password) throws IOException {
        ...
        Response r = contactProxy(new CryptedLoginRequest(username, challenge)); // Init
                                                                                 // Login-Request

        if (!(r instanceof CryptedLoginResponse)) { // Answer was not according
                                                    // to protocoll
        ...
        if (!clr.getClChallenge().equals(challenge)) { // Challenge was not
                                                       // returned correctly
    ...
    public Response download(String filename) throws IOException {
        ...
        if (!(resp instanceof DownloadTicketResponse)) { // Probably
                                                         // messageresponse
                                                         // containing errormsg
            return resp;
        }
        ...
        if (!(resp2 instanceof DownloadFileResponse)) { // Probably
                                                        // messageresponse
                                                        // containing errormsg
            return resp;
        }
    ...
    public MessageResponse upload(String filename) throws IOException {
        try {
            return (MessageResponse) contactProxy(new UploadRequest(filename, 0, FileUtils.getBytesFromFile(path + "/"
                    + filename)));
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void connectToProxy(String username, String password) throws IOException {
        ...
        proxyChannel.getOperators().add(
                new BiDirectionalRsaOperator(proxyKeyPath, keyPath + username + ".pem", password));
